...
export const FloatingDock = ({
  ...
  items: { title: string; icon: React.ReactNode; href: string; active: boolean; onClick?: (e: React.MouseEvent<HTMLAnchorElement, MouseEvent>) => void }[];
    ...
    <div className={`relative block md:hidden ${className}`}>
      <AnimatePresence>
        {open && (
          ...
          >
            {items.filter(item => item.active).map((item, idx) => (
              <motion.div
                key={item.title}
      ...
      >
        <RxHamburgerMenu className="h-5 w-5 text-neutral-400" />
      </button>
    </div>
  );
};
